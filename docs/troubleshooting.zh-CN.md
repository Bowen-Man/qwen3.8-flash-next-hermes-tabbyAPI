...
# 10. 理解 `sysmem_recurrent_cache` 与 `sysmem_kv_cache`
...
它们是两个不同的 cache，不应简单理解成“把多少 GB active GPU KV 永久搬进 RAM”。

`sysmem_recurrent_cache` 关联 recurrent/hybrid attention state；`sysmem_kv_cache` 提供 second-tier KV reuse/management 容量。二者都会与 CPU/RAM bandwidth 和 runtime cache state 发生交互。

32K/64K active workload 仍需要 GPU-resident working state、attention workspace 和 runtime allocations，所以增加 system-memory cache 不会线性降低 `nvidia-smi` 的 VRAM 数字。

# 11. 更大的 system-memory cache 不保证更快

参考机器的历史测试明显是非单调的：64K 表现很好的配置，在 32K 未必最好。

`cuda_malloc_async=false` 历史例子：
...
这些是单次 observation，不是全局最优证明。cached-prefix TTFT 的 run-to-run variation 也明显比 first-pass TTFT 大。

一次只调一个参数，并用 `scripts/benchmark_chat.py --runs 3` 做更可靠比较。
...
在 `10R/4K @ 64K` 下，打开 allocator 的结果与此前两次 `false` 处于同一性能区间。如果出现 intermittent CUDA OOM 或 allocator instability，再固定其他参数测试 `false`。
...
这是一套经过实测的 known-good reference，并不是所有硬件上的全局最优。示例 endpoint 只绑定 localhost；不要在 `disable_auth: true` 时暴露到公网。
