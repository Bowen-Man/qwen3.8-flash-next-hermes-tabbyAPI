...
在本次 12 GB GPU 环境中，该配置下模型加载完成后的 dedicated GPU memory 静态占用约为：

```text
10.5 GB
```

这个数字会受到硬件、驱动、CUDA、ExLlamaV3 版本和其他 GPU 程序的影响，不能作为所有机器的固定结果。
