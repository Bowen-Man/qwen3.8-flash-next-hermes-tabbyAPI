...
# 10. Understanding `sysmem_recurrent_cache` and `sysmem_kv_cache`
...
These are separate caches. Do not interpret them as a simple amount of active GPU KV permanently moved into RAM.

`sysmem_recurrent_cache` is relevant to recurrent/hybrid attention state, while `sysmem_kv_cache` provides second-tier capacity for KV reuse/management. Both interact with CPU/RAM bandwidth and runtime cache state.

The active 32K/64K workload still needs GPU-resident working state, attention workspace and runtime allocations, so increasing system-memory cache does not produce a linear decrease in `nvidia-smi` VRAM usage.

# 11. Larger system-memory caches are not automatically faster

Historical tests on the reference machine were non-monotonic. A configuration that performed well at 64K did not necessarily perform best at 32K.

Examples from the `cuda_malloc_async=false` experiments:
...
These are single-run observations, not proof of universal optima. Cache reuse also has more run-to-run variation than first-pass TTFT.

Tune one parameter at a time and use `scripts/benchmark_chat.py --runs 3` for more defensible comparisons.
...
At `10R/4K @ 64K`, the allocator-enabled run fell inside the same performance envelope as two earlier `false` runs. If you encounter intermittent CUDA OOM or allocator-related instability, benchmark `false` while keeping all other settings fixed.
...
This is a tested known-good reference, not a universal optimum. The example endpoint is localhost-only; do not expose `disable_auth: true` publicly.
