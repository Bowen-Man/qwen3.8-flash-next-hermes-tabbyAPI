...
| KV cache | FP16 |
| System-memory caches | 10 GB recurrent + 4 GB second-tier KV |
| Agent | Hermes Agent |
...
Reference EXL3 weights are from [turboderp/Qwen3.8-Flash-Next-exl3](https://huggingface.co/turboderp/Qwen3.8-Flash-Next-exl3). The model card currently states that these quants require **ExLlamaV3 v1.4.5 or newer**; the reference machine in this guide uses ExLlamaV3 1.4.8.
...
# 11. System-memory recurrent and KV caches

The current known-good reference uses:
...
`sysmem_recurrent_cache` and `sysmem_kv_cache` are separate controls. They should not be interpreted as a simple amount of active GPU cache permanently moved into RAM.

The tested system uses large system RAM deliberately for heterogeneous inference. Cache performance is workload- and context-dependent, and larger allocations are not automatically faster.

`cuda_malloc_async: true` is part of the current known-good reference. On the tested 10R/4K 64K workload, turning it on did not materially change TTFT/decode compared with earlier `false` runs. If you encounter intermittent CUDA OOM or allocator instability, test `false` while holding other settings constant.

For tuning methodology and measured data, see:
...
Current Hermes releases require local Agent models to expose at least a **64K context window**. The 81,920-token server configuration used here satisfies that requirement while leaving useful room for system prompts, tool schemas, history and tool results.
...
The current 12 GB reference deployment uses heterogeneous memory approximately as follows:
...
Current reference memory settings:
